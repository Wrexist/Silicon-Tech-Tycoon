...
- **The store has ONE seam, and two backends behind it.** `src/state/storeKitBridge.ts` defines
  ...
  SPM-capable version needs Capacitor 8. Add RevenueCat to the **App target in Xcode**, never to
  `CapApp-SPM/Package.swift` (regenerated by `npx cap sync ios`). New purchase behaviour goes in the
  adapter, never in the callers — if a change needs `proStore.ts` edited, the adapter is leaking.
  Setup, the customer-migration story and the sandbox matrix: `appstore/REVENUECAT_SETUP.md`.
