...
// The boundary belongs to the WIDER mode: 800 is tablet, 1100 is wide.
describe("layoutModeForWidth", () => {
  it("maps the phone range", () => {
    for (const w of [320, 390, 540, 699, 700, 799]) expect(layoutModeForWidth(w)).toBe("phone");
  });

  it("maps the tablet range, inclusive of the boundary", () => {
    for (const w of [800, 820, 1099]) expect(layoutModeForWidth(w)).toBe("tablet");
  });
