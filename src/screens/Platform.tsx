...
import { Button, Card, Stat, SectionHeader, SectionRail } from "../design/primitives.tsx";
...
/** The classic sheet: chrome (brand + title, and the Done close affordance) around the shared body,
 *  so the flag-off Company sub-tab and the routed Silicon-2.0 page can never drift apart. */
 ...
 *  and by the classic sheet above, so the two can never drift. A `section` narrows the body to one
...
export function PlatformPanel({
  ...
  // --- Blocks (defined once, then grouped per section, so the sheet and the page render the exact
  //     same nodes and the flag-off path cannot drift). ---
  ...
  const offerPending = state.pendingLicenseOffer;
  const offerCard = offerPending && (() => {
    const offer = offerPending;
  ...
  // The routed page: a rail beside the one section it names. Each block appears in exactly one
  // section, in its original relative order. Overview picks up the blocks the contract does not name.
  if (section && onSection) {
    const bySection: Record<PlatformSection, ReactNode> = {
      overview: <>{osNameCard}{philosophyCard}{versionCard}{securityCard}</>,
      services: <>{osIncomeStat}{reachCard}</>,
      ecosystem: <>{featuresCard}</>,
      licensing: <>{offerCard}{licenseesCard}</>,
      developers: <>{appStoreCard}</>,
      distribution: <>{installedBaseStat}</>,
    };
    return (
      <>
        <div className="ds-panel">
          ...
          />
          <div className="ds-panel__body">{bySection[section]}</div>
        </div>
        {overlays}
      </>
    );
  ...
  // The classic sheet path: every block, in its original relative order (below 800px this is the
  // same single column the division always was).
  return (
    <>
      {osNameCard}
      {philosophyCard}
      <div className="plat__stats">{installedBaseStat}{osIncomeStat}</div>
      {reachCard}
      {versionCard}
      {securityCard}
      {appStoreCard}
