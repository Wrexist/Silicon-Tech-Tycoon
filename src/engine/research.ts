...
/** Memoized result of completableProjectCount() — the catalog is static, so compute once. */
let completableCountCache: number | null = null;

/** The maximum number of projects a single company can ever complete in one run. The "research
 *  everything" achievement targets this, since completing the full RESEARCH_PROJECTS list is
 *  impossible once forks exist: each fork group allows only ONE sibling, and a non-forked project
 *  whose `requires` names a fork sibling (the doctrine tier-2s) is only reachable when THAT sibling
 *  was the one chosen. Computed exactly: for every combination of one-choice-per-fork, take the
 *  greedy unlock closure (monotone, so greedy is maximal for a fixed choice) and return the largest.
 *  Pure — a deterministic fold over the static catalog, memoized. */
export function completableProjectCount(): number {
  if (completableCountCache !== null) return completableCountCache;
  // Fork groups in catalog order, each with its sibling ids in catalog order.
  const groups = new Map<string, ProjectId[]>();
  for (const p of RESEARCH_PROJECTS) {
    if (p.fork) (groups.get(p.fork) ?? groups.set(p.fork, []).get(p.fork)!).push(p.id);
  }
  const forkNames = [...groups.keys()];
  let best = 0;
  const tryChoices = (i: number, chosen: Map<string, ProjectId>): void => {
    if (i < forkNames.length) {
      for (const sibling of groups.get(forkNames[i])!) {
        chosen.set(forkNames[i], sibling);
        tryChoices(i + 1, chosen);
      }
      chosen.delete(forkNames[i]);
      return;
    }
    // Greedy closure under this fork assignment: research everything unlockable, repeatedly.
    const completed: ProjectId[] = [];
    let progressed = true;
    while (progressed) {
      progressed = false;
      for (const p of RESEARCH_PROJECTS) {
        if (p.fork && chosen.get(p.fork) !== p.id) continue; // not this run's pick
    ...
    if (completed.length > best) best = completed.length;
  };
  tryChoices(0, new Map());
  completableCountCache = best;
  return best;
}
