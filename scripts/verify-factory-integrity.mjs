...
await p.getByText('Nothing to erase here',{exact:true}).waitFor();
if(await p.getByRole('button',{name:'Undo the last build action',exact:true}).count())throw Error('Empty erase added Undo history');
...
await p.screenshot({path:'artifacts/factory-fixes/images/fallback-undo.png'});
...
console.log('PASS: actual fallback grid purchase, separate expansion, Undo, exact wallet/ownership, office preservation, reload; no page errors.');
