// "New features" App Store screenshots — the same premium 3D-perspective marketing frame as
// shots-store.mjs, for the systems added since the last store set: the fully-decorated 3D Factory
// line, the Silicon Awards ceremony, a Rival Strike duel, and a Side-Order commission. Each live
// screen is captured from the showcase staging save (+ per-frame overlay injection) and composed
// into the tilted-titanium frame. Output: app-store-screenshots/store/NN-*.png at 1284×2778.
//
//   npm run build && npm run preview -- --port 5199 &
//   npm run shots:stage:showcase        # → /tmp/silicon-showcase.json (+ -overlays.json)
...
let baseSave, overlays;
try {
  baseSave = JSON.parse((await readFile("/tmp/silicon-showcase.json")).toString());
  overlays = JSON.parse((await readFile("/tmp/silicon-showcase-overlays.json")).toString());
} catch {
  console.error("Showcase staging not found. Run: npm run shots:stage:showcase");
  process.exit(1);
}
// Clone the base save and apply a per-frame mutation (inject exactly one overlay, etc.).
...
async function page(saveJson) {
  ...
  // Pause immediately so no organic Rival Strike fires and hijacks a factory/side-order frame; the
  // injected overlays (awards/strike) mount from persisted state regardless, and the 3D line keeps
  // animating off the render loop (an active build) even while the sim clock is paused.
...
// Enter the Factory world + open the full-screen factory mode, dismissing any first-run coach.
const openFactory = async (p) => {
  ...
  await p.click('button[aria-label="Open factory mode"]', { timeout: 6000 });
  await p.waitForTimeout(900);
...
const FRAMES = [
  {
    ...
    sub: "Lay conveyor, place machines, and watch a real 3D production floor run your builds.",
    mut: (s) => { s.pendingStrike = null; s.pendingAwards = null; s.pendingSideOrder = null; },
  ...
  },
  {
    ...
    mut: (s) => { s.pendingAwards = overlays.awards; s.pendingStrike = null; s.pendingSideOrder = null; },
    shoot: async (p) => { await p.waitForTimeout(1400); }, // ceremony auto-mounts on load
  },
  {
    ...
    mut: (s) => {
      s.pendingStrike = overlays.strike;
      s.rivalReleases = [...(s.rivalReleases || []), overlays.rivalRelease];
      s.pendingAwards = null; s.pendingSideOrder = null;
    },
    shoot: async (p) => { await p.waitForTimeout(1400); }, // strike card auto-mounts on load
  },
  {
    raw: "sideorder", head: 'Take the <span class="ac">contract</span>', hue: 200,
    sub: "Clients commission runs on your line. Meet the deadline, bank the fee, keep it busy.",
    mut: (s) => { s.pendingSideOrder = overlays.sideOrder; s.pendingStrike = null; s.pendingAwards = null; },
    shoot: async (p) => {
      await openFactory(p);
      await p.evaluate(() => document.querySelector(".fmode__sideorder")?.scrollIntoView({ block: "center" })).catch(() => {});
...
for (const fr of FRAMES) {
  ...
  try {
    await fr.shoot(p);
    // Freeze animations only AFTER the shoot (the factory line must animate to look alive first).
    ...
    await p.screenshot({ path: resolve(rawDir, `feat-${fr.raw}.png`), clip: { x: 0, y: 0, width: 390, height: 844 } });
...
// ---- compose into the 3D marketing frame (identical to shots-store.mjs) ----
...
// Continue the store numbering: the existing set ends at 10, so these are 11–14.
let i = 11;
for (const fr of FRAMES) {
  const b64 = (await readFile(resolve(rawDir, `feat-${fr.raw}.png`))).toString("base64");
...
await browser.close();
console.log(`done. ${FRAMES.length} feature frames at ${SIZE.w}×${SIZE.h}.`);
