...
// doctrines). Output: app-store-screenshots/store/NN-*.png at 1290×2796 (6.7" App Store slot).
//
...
const FRAMES = [
  ...
  { raw: "decorate", head: 'Make it <span class="ac">yours</span>', sub: "Design your studio. Drag in 60+ pieces of parametric furniture.", hue: 168,
    shoot: async (p) => { await tab(p, "Office"); await p.waitForTimeout(500); await top(p); await p.evaluate(() => [...document.querySelectorAll("button")].find((e) => /Decorate/.test(e.textContent || ""))?.click()); await p.waitForTimeout(2000); } },
