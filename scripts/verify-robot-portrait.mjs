// Diagnostic camera only; not the normal play camera.
...
await ctx.addInitScript(()=>{
 ...
 const render=renderer.render;renderer.render=function(scene,camera){
  if(camera.isPerspectiveCamera) {
    const robot=scene.getObjectByName('office-robot-body');
    if(robot) {
      scene.updateMatrixWorld(true);
      const center=camera.position.clone(); robot.getWorldPosition(center); center.y+=1.25;
      const rotation=camera.quaternion.clone(); robot.getWorldQuaternion(rotation);
      const offset=camera.position.clone().set(0.5,0.25,5.2).applyQuaternion(rotation);
...
const output=process.env.SHOTS_OUTPUT || 'artifacts/release-final';
await mkdir(output,{recursive:true});
await p.screenshot({path:`${output}/portrait-${theme}.png`});
console.log(JSON.stringify(diagnostic));

if(errors.length)throw Error(errors.join('\n'));
console.log(`PASS ${theme}: diagnostic robot close-up rendered without page errors (camera overridden for inspection).`);
