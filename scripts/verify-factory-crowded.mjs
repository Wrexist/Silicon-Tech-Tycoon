...
const raw=JSON.parse(await readFile('C:/tmp/silicon-showcase.json','utf8'));
...
await mkdir('artifacts/factory-fixes/crowded/images',{recursive:true});
const results=[];
for(const theme of ['dark','light']){
 ...
 await p.screenshot({path:`artifacts/factory-fixes/crowded/images/${theme}-320.png`});
 ...
 for(const name of ['Stats','Style','Build']) {
  ...
  await p.screenshot({path:`artifacts/factory-fixes/crowded/images/${theme}-${name.toLowerCase()}-320.png`});
 ...
 await p.screenshot({path:`artifacts/factory-fixes/crowded/images/${theme}-390.png`});
 const layouts=[];
 ...
 await p.getByRole('button',{name:'Recenter view',exact:true}).click();await p.waitForTimeout(400);
 await p.screenshot({path:`artifacts/factory-fixes/crowded/images/${theme}-max-expansion.png`});
...
await import('node:fs/promises').then(fs=>fs.writeFile('artifacts/factory-fixes/crowded/browser.json',JSON.stringify(results,null,2)));
