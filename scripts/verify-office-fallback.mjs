// Production-build Reduce Motion and no-WebGL office regression checks.
...
for(const mode of ['reduce-motion','no-webgl']) {
 ...
 window.__THREE_DEVTOOLS__={dispatchEvent(e){const r=e.detail;if(!r?.isWebGLRenderer)return;const render=r.render;r.render=function(scene,camera){const result=render.call(this,scene,camera);if(camera.isPerspectiveCamera)window.__motionScene=scene;return result;};}};
 ...
 if(mode==='reduce-motion'){
  await page.waitForFunction(()=>window.__motionScene?.getObjectByName('office-robot-body'));
  const poses=()=>page.evaluate(()=>{const poses=[];window.__motionScene.traverse(o=>{if(o.name==='office-robot-body')o.traverse(b=>poses.push([...b.position.toArray(),...b.quaternion.toArray(),...b.scale.toArray()]));});return poses;});
  const before=await poses();await page.waitForTimeout(900);const after=await poses();
  if(before.length===0||JSON.stringify(before)!==JSON.stringify(after))throw Error('Reduce Motion still animates staff bones');
  console.log('PASS Reduce Motion: staff transforms stay unchanged across rendered frames');
 }
 await page.screenshot({path:`artifacts/office-motion/${mode}.png`});
